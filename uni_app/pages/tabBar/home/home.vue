	...
	<view>
		...
		<view class="goods-list">
			...
			<view class="product-list">
				<view class="product" v-for="item in productList" :key="item._id" @tap="toGoods(item)">
					<image mode="widthFix" :src="'http://localhost:3001/' + item.images[0]"></image>
					<view class="name">{{ item.name }}</view>
					<view class="info">
						<view class="price">￥{{ item.price }}</view>
						<view class="slogan">{{ item.producer }}</view>
					</view>
				</view>
			</view>
...
export default {
	data() {
		return {
			...
			// 分类菜单
			categoryList: [
				{ id: 1, name: '宠物', img: '/static/img/category/1.png' },
				{ id: 2, name: '服饰', img: '/static/img/category/2.png' },
				{ id: 3, name: '主粮', img: '/static/img/category/3.png' },
				{ id: 4, name: '零食', img: '/static/img/category/4.png' },
				{ id: 5, name: '玩具', img: '/static/img/category/5.png' },
				{ id: 6, name: '日用', img: '/static/img/category/6.png' },
				{ id: 7, name: '户外', img: '/static/img/category/7.png' },
				{ id: 8, name: '宠物窝', img: '/static/img/category/8.png' }
			],
			Promotion: [],
			//猜你喜欢列表
			productList: [],
	...
	methods: {
		...
		toSearch() {
			uni.navigateTo({
				url: '../../goods/searchgoods/searchgoods'
			});
		},
		//轮播图跳转
		toSwiper(e) {
		...
		toGoods(item) {
			uni.navigateTo({
				url: '../../goods/goods?cid=' + encodeURIComponent(JSON.stringify(item)),
			});
		},
		//轮播图指示器
		swiperChange(event) {
