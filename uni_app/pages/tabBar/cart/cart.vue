<template>
	<view>
		...
		<view v-if="showHeader" class="header" :style="{ position: headerPosition, top: headerTop }"><view class="title">购物车</view></view>
		<!-- 占位 -->
		...
		<view class="goods-list">
			...
			<view class="row" v-for="(row, index) in goodsList" :key="index">
				<!-- 删除按钮 -->
				<view class="menu" @tap.stop="deleteGoods(row.id)" @click="Delete(row.id)"><view class="icon shanchu"></view></view>
				<!-- 商品 -->
				<view
					class="carrier"
				...
				>
					...
					<!-- 商品信息 -->
					<view class="goods-info" @tap="toGoods(row.goods)">
...
export default {
	...
	methods: {
		shopping() {
			...
			uni.request({
				url: 'http://localhost:3001/shoppingCart/getShoppingCart', //地址。
				data: {
					userId: userId[0]._id
				},
				success: res => {
					...
					if (res.data) {
						for (let i = 0; i < res.data.length; i++) {
							this.goodsList.push({
								id: res.data[i]._id,
								img: 'http://localhost:3001' + res.data[i].goodsId.images[0],
								name: res.data[i].goodsId.name,
								spec: res.data[i].spec,
								price: res.data[i].goodsId.price,
								number: res.data[i].number,
								selected: res.data[i].selected,
								goods: res.data[i].goodsId
							});
						}
					}
				}
		...
		//商品跳转
		toGoods(goods) {
			uni.showToast({ title: '商品' + goods._id, icon: 'none' });
			uni.navigateTo({
				url: '../../goods/goods?cid=' + encodeURIComponent(JSON.stringify(goods))
			});
		},
		//-------------------------------------------------------------------------------
		//跳转确认订单页面
		toConfirmation() {
			...
			for (let i = 0; i < len; i++) {
				if (this.goodsList[i].selected) {
					tmpList.push({
						id:this.goodsList[i].goods._id,
						img:this.goodsList[i].img,
						name:this.goodsList[i].name,
						spec:this.goodsList[i].spec,
						price:this.goodsList[i].price,
						number:this.goodsList[i].number,
						_iD:this.goodsList[i].id
					});
				}
			}
			if (tmpList.length < 1) {
