			...
			<view class="product-list">
				<view class="product" v-for="(goods) in goodsList" :key="goods.goods_id" @tap="toGoods(goods)">
					...
					<view class="info">
						...
						<view class="slogan">{{goods.producer}}</view>
					</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods:{
			getGoods() {
				const classify = uni.getStorageSync('catName');
				uni.request({
			...
			//商品跳转
			toGoods(e){
				uni.showToast({title: '商品'+e._id,icon:"none"});
				uni.navigateTo({
					url: '../goods?cid=' + encodeURIComponent(JSON.stringify(e)),
				});
			},
			//排序类型
			select(index){
...
<style lang="scss">
	...
	.header{
		...
		.target{
			...
			&.on{
				color: #555;
				border-bottom: 4upx solid #398BFB;
				font-weight: 600;
				font-size: 30upx;
			}
