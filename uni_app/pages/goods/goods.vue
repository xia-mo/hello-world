	...
	<view>
		...
		<view class="swiper-box">
			<swiper circular="true" autoplay="true" @change="swiperChange">
				<swiper-item v-for="swiper in swiperList" :key="swiper"><image :src="'http://localhost:3001/' + swiper"></image></swiper-item>
			</swiper>
...
export default {
	...
	onLoad(option) {
		...
		const item = JSON.parse(decodeURIComponent(option.cid))
		this.goodsData.id=item._id;
		this.goodsData.name=item.name;
		this.goodsData.price=item.price;
		this.swiperList=item.images;
	},
	onReady(){
	...
	methods: {
		...
		shopping(){
			...
			if (this.goodsData.id) {//上传服务器。
				uni.request({
					url: 'http://localhost:3001/shoppingCart/addShoppingCart', //地址。
					method: 'POST',
					data: {
						    userId: userId[0]._id,
						    goodsId: this.goodsData.id,
						    spec: '规格:'+this.goodsData.spec[this.selectSpec], //规格
						    number: this.goodsData.number,//数量
						    selected: false,//是否选择
					},
					success: res => {
		...
		toConfirmation(){
			...
			let goods = {
					...
					img:'http://localhost:3001'+this.swiperList[0],
