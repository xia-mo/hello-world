<template>
	<view>
		...
		<!-- 账号密码输入框 -->
		<view class="form">
			<view class="username"><input placeholder="请输入手机号" v-model="phoneNumber" placeholder-style="color: rgba(255,255,255,0.8);" /></view>
			<view class="password"><input placeholder="请输入密码" v-model="passwd" password="true" placeholder-style="color: rgba(255,255,255,0.8);" /></view>
...
export default {
	...
	methods: {
		...
		doLogin() {
			...
			//密码验证规则 最少6位，包括至少1个大写字母，1个小写字母，1个数字，1个特殊字符 例 Aa123#
			if (!/^.*(?=.{6,})(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*? ]).*$/.test(this.passwd)) {
				uni.showToast({ title: '请填写正确密码', icon: 'none' });
				return false;
			}

			// 上传服务器比对。
			uni.request({
				url: 'http://localhost:3001/users/login', //地址。
				method: 'POST',
				data: {
