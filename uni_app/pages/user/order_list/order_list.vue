					...
					<view class="order-info">
						...
						<view class="right">
							...
							<view class="price-number">
								...
								x
								<view class="number">{{ row.numner }}</view>
							</view>
						</view>
					</view>
					<view class="detail">
...
export default {
	data() {
		return {
			...
			//待付款 unpaid   待发货 back  待收货 unreceived  待评价 received  退货处理中 refunds
			//交易已完成 completed     订单已取消 cancelled
			//订单列表 演示数据
			orderList: [[], [], [], [], [], []],
			list: [],
			tabbarIndex: 0
	...
	methods: {
		getOrder() {
			...
			if (userId) {
				// 上传服务器。
				uni.request({
					url: 'http://localhost:3001/orders/getOrders', //地址。
					method: 'POST',
					data: {
						userId: userId[0]._id,
						state: '待发货'
					...
					success: res => {
						...
						if (res.data) {
							for (let i = 0; i < res.data.length; i++) {
								this.orderList[0].push({//全部
									type: 'back',
									ordersn: 2,
									goods_id: res.data[i]._id,
									img: 'http://localhost:3001' + res.data[i].orderGoodsId.goodId.images[0],
									name: res.data[i].orderGoodsId.goodId.name,
									price: res.data[i].orderGoodsId.goodId.price,
									payment: res.data[i].orderGoodsId.goodId.price * res.data[i].orderGoodsId.count,
									freight: 12.0,
									spec: '规格:大',
									numner: res.data[i].orderGoodsId.count
								});
								this.orderList[2].push({//待发货
									type: 'back',
									ordersn: 2,
									goods_id: res.data[i]._id,
									img: 'http://localhost:3001'+res.data[i].orderGoodsId.goodId.images[0],
...
.topTabBar {
	...
	.grid {
		...
		.text {
			...
			&.on {
				color: #398bfb;
				border-bottom: solid 4upx #398bfb;
			}
		}
	}
}
